...
export async function register(req, res) {
  ...
  const { lastID } = db.run('INSERT INTO users (email, password_hash) VALUES (?,?)', email, hash)
  const token = jwt.sign({ sub: lastID }, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN })
  return res.status(201).json({ token, user: { id: lastID, email } })
}
...
export async function login(req, res) {
  ...
  if (!email || !password) return res.status(400).json({ message: 'email/password required' })
  const db = await getDb()
  const row = db.get('SELECT id, password_hash FROM users WHERE email = ?', email)
